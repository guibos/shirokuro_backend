...
from context.shared.infrastructure.bus.command.bus.exceptions.command_not_registered_error import \
    CommandNotRegisteredError
from context.shared.infrastructure.bus.command.bus.exceptions.command_registered_after_dispatch import \
...
class CommandBusSync(CommandBus):
    ...
    def _get_handler(self, command: Command):
        try:
            ...
            self._logger.critical(f'Command "%s" has not been registered.',
                                  type(command).__name__)
            raise CommandNotRegisteredError from e

    def _guard_has_been_called(self):
        if self._ask_has_been_called:
            raise CommandRegisteredAfterDispatch(
                'Trying to register a new handler after some command has been dispatched'
            )
